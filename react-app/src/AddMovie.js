...
class AddMovie extends Component {
	...
	constructor(props) {
		...
		this.processCreateRequest = this.processCreateRequest.bind(this);
	...
	tryCreateMovie() {
		...
		xhr.send(JSON.stringify({ "title": _title, "rating": _rating, "plot": this.state.plot, "director": this.state.director, "runtime": this.state.runtime, "release": this.state.release, "genre": _genre }));
		xhr.addEventListener("readystatechange", this.processCreateRequest, false);
	}

	processCreateRequest() {
		if (xhr.readyState === 4 && xhr.status === 200) {
			this.changeState( { title: "na" } )
			this.changeState( { rating: "na" } )
			this.changeState( { plot: "na" } )
			this.changeState( { runtime: "na" } )
			this.changeState( { release: "na" } )
			this.changeState( { director: "na" } )
			this.changeState( { genre: 'na' } )
		}
	}
	...
	render() {
		...
		return (
			<body>
			<span className="saveButton" onClick={this.tryCreateMovie}>
				&nbsp;&nbsp;Save&nbsp;&nbsp;
			</span>
